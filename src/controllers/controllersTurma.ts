...
export class ControllersTurma {
  async create(request: Request, response: Response) {
    const { data_inicio, data_fim, horas_aula_dia, fk_curso, turno } = request.body
    const result = await service.create(data_inicio, data_fim, horas_aula_dia, fk_curso, turno)
  ...
  async filterTurno(request: Request, response: Response) {
    ...
    if (result.length < 1) {
      return response.status(204).json("Nenhuma turma com esse turno foi encontrada!")
    }
    return response.status(200).json(result)
  }
}
