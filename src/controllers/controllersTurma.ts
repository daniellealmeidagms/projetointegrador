...
export class ControllersTurma {
  async create(request: Request, response: Response) {
    const { data_inicio, data_fim, horas_aula_dia, fk_curso } = request.body
    const result = await service.create(data_inicio, data_fim, horas_aula_dia, fk_curso)
  ...
  async update(request: Request, response: Response) {
    const { id_turma } = request.params
    const { data_inicio, data_fim, horas_aula_dia, fk_curso } = request.body
    const result = await service.update(id_turma, data_inicio, data_fim, horas_aula_dia, fk_curso)
    if (result instanceof Error) {
