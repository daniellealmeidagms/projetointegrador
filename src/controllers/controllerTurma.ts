...
export class ControllerTurma {
  ...
  async create(request: Request, response: Response) {
    const { data_inicio, data_fim, horas_aula_dia } = request.body
    const result = await service.create(data_inicio, data_fim, horas_aula_dia)
  ...
  async update(request: Request, response: Response) {
    const { id_turma } = request.params
    const { data_inicio, data_fim, horas_aula_dia } = request.body
    const result = await service.update(id_turma, data_inicio, data_fim, horas_aula_dia)
    if (result instanceof Error) {
      return response.status (404).json(result.message)
    }
    return response.status(200).json(result)
